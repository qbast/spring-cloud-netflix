...
import org.springframework.cloud.netflix.zuul.filters.RouteLocator.RouteSpec;
...
public class ProxyRouteLocatorTests {
	...
	public void testGetMatchingPath() throws Exception {
		...
		routeLocator.getRouteLocations(); // force refresh
		RouteSpec route = routeLocator.getMatchingRoute("/foo/1");
	...
	public void testGetMatchingPathWithPrefix() throws Exception {
		...
		this.properties.init();
		routeLocator.getRouteLocations(); // force refresh
		RouteSpec route = routeLocator.getMatchingRoute("/proxy/foo/1");
		assertEquals("foo", route.getLocation());
	...
	public void testGetMatchingPathWithServletPath() throws Exception {
		ProxyRouteLocator routeLocator = new ProxyRouteLocator(this.discovery, this.properties, "/app"
		);
		this.properties.getRoutes().put("foo", new ZuulRoute("/foo/**"));
		this.properties.init();
		routeLocator.getRouteLocations(); // force refresh
		RouteSpec route = routeLocator.getMatchingRoute("/app/foo/1");
		assertEquals("foo", route.getLocation());
		assertEquals("/1", route.getPath());
	}
	...
	public void testGetRoutes() {
		...
		this.properties.init();
		Map<String, String> routesMap = routeLocator.getRouteLocations();
